...
export default function GameModeSelect() {
  ...
  const civilizations = [
    { id: 'rome', name: '로마', specialty: '군사 확장에 특화', color: 'from-red-700 to-red-900', icon: <Sword size={28} /> },
    { id: 'china', name: '중국', specialty: '과학과 경이 건설에 보너스', color: 'from-yellow-700 to-yellow-900', icon: <Star size={28} /> },
    { id: 'egypt', name: '이집트', specialty: '문화와 경이 건설에 특화', color: 'from-amber-700 to-amber-900', icon: <Star size={28} /> },
    { id: 'mongol', name: '몽골', specialty: '기병 유닛과 정복에 강점', color: 'from-green-700 to-green-900', icon: <Sword size={28} /> },
    { id: 'england', name: '영국', specialty: '해군과 식민지 확장에 보너스', color: 'from-blue-700 to-blue-900', icon: <Globe size={28} /> },
    { id: 'france', name: '프랑스', specialty: '문화와 외교에 특화', color: 'from-indigo-700 to-indigo-900', icon: <Users size={28} /> },
    { id: 'japan', name: '일본', specialty: '군사와 생산에 균형된 능력', color: 'from-rose-700 to-rose-900', icon: <Sword size={28} /> },
    { id: 'aztec', name: '아즈텍', specialty: '종교와 인적 자원 활용에 특화', color: 'from-emerald-700 to-emerald-900', icon: <User size={28} /> }
  ];

  const mapTypes = [
    { id: 'continents', name: '대륙', description: '여러 개의 대륙으로 구성된 지도', icon: <Map size={28} /> },
    { id: 'islands', name: '섬', description: '크고 작은 섬들로 구성된 지도', icon: <Map size={28} /> },
    { id: 'pangaea', name: '판게아', description: '하나의 거대한 대륙으로 구성된 지도', icon: <Map size={28} /> },
    { id: 'inland_sea', name: '내해', description: '중앙에 바다가 있는 지도', icon: <Map size={28} /> }
  ];
  ...
  const isCurrentStepSelected = () => {
    switch (step) {
      ...
      case 4: return !!selectedMapType || true; // 맵 타입은 기본값이 있으므로 항상 true
          ...
          <div className="w-full max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              {mapTypes.map((mapType, index) => (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-center mb-2">{mapType.name}</h3>
                  <p className="text-gray-400 text-center text-sm">{mapType.description}</p>
                </div>
              ))}
            </div>
            
            <h3 className="text-2xl font-bold mb-4 text-center">지도 크기</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {mapSizes.map((map, index) => (
                ...
                >
                  <h3 className="text-lg font-bold text-center mb-1">{map.name}</h3>
                  <p className="text-gray-300 text-center text-xs mb-1">{map.size}</p>
                  <p className="text-gray-400 text-center text-xs">{map.players}</p>
                </div>
              ))}
            </div>
          </div>
